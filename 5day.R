...
# JSON(Jara Script Object Notation)
...
idx <- which(df.xlsx[, 1:2] == "NA", arr.ind = TRUE)
                 # arr.ind = TRUE : 해당 조건의 행/열 값을 확인할때
idx
...
#   -분석을 해야할자료를 가지고 있을 때 1차적으로 해야 할 일은
#    해당 자료가 어떤 분류에 속하는지를 파악하는 일이다.
#
# - 자료 특성에 따른 분류
#   분석 대상 자료의 특성에 따라 범주형 자료와 연속형 자료로 구분한다.
...
#   - 산술연산이 의미가 있냐 없냐 차이임 / 없는거 -> 범주형
#
# 2. 연속형 자료 (numerical data)
#   - 크기가 있는 숫자들로 구성된 자료
#   - 연속형 자료의 값들은 대소비교, 산술연산이 가능하기 때문에
#     분석 방법이 존재한다.
#   - 산술연산이 의미가 있는거 / -> 연속형
#
...
#        두 개이상의 변수로 구성된 자료
#        특별히 두개의 변수로 구성된 자료를 이변량 자료(bivariate data)
#
...
#               이상 값(평균이80인데 30인 사람도있음) 에 영향을 받는다.
...
hist(dist, main = "Histogram for 제동거리",
     ...
     las=2,breaks=5)    # las : x축 글자 1->세로로, 2->가로로
