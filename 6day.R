...
# - 다중변수 자료는 2차원 형태, matrix나 dataframe에 저장하여 분석
# - 다중변수 자료에서 변수는 열(column),
#   개별 관측값들이 행(row)로 구성
#
# 산점도(scatter plot) : 2개의 변수로 구성된 자료의 분포를 알아보는 그래프
#                        관측값들의 분포를 2개의 변수 사이의 관계를
#                        파악할 수 있는 기법
# 상관관계를 분석 -> 회귀분석
#
# - 산점도는 두 변수의 데이터 분포를 나타내는 것이기 때문에
#   두 개의 변수에 대한 자료가 필요
# - 산점도는 관측값들의 분포를 보면서 두 변수 사이의 관련성을
#   확인하는데 사용 
...
# - 다중 산점도는 대각선을 기준을 오른쪽 위의 산점도들과 왼쪽 아래의
#   산점도들이 대칭을 이룬다.
...
pairs(target,main="multi plots")  # 방법1


plot(target,main="multi plots")   # 방법2 
