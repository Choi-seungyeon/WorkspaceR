...
z <- matrix(1:20, nrow=4, ncol=5, byrow=T) #byrow: 보통은 열우선인데 byrow쓰면 행우선으로 
...
rownames(score)
colnames(score)
colnames(score)[2]   #함수의 결과에 대해서 2번째 값


c <- colnames(score) #colnames(score)[2] /풀어쓰기
c[2]
...
unique(iris[,5])    #중복되는 값을 모두 제외시켜주는 함수?

table(iris[,5])            # 그룹별로 개수 카운트? 도수분포표?
...
iris.m <- as.matrix(iris)   #iris라는 데이터프레임을 매트릭스로 변환
class(iris.m)
str(iris.m)
dim(iris.m)
